...
// Night-vision "trail cam" background: a fixed green monochrome
// palette (deliberately NOT read from the live Theme:: palette — the
// whole point of the effect is the NV-goggle green look, which should
// stay put regardless of which color theme is active), scanlines,
// static grain, a cheap corner-cut vignette standing in for the
// circular goggle tunnel, and a rotating cast of real-world (and
// internet-famous) cryptid silhouettes pacing through — each species
...
void drawCryptidCam(TFT_eSPI& t, uint32_t now, int yStart, int yEnd) {
    ...
    struct Walker {
        float x, speed, phase; int8_t dir; int h; uint8_t species;
        uint32_t nextJump, glitchUntil;   // species 6 only
    };
    static const uint8_t N = 3;
    static Walker  walkers[N];
    static bool    inited = false;
    ...
    const uint16_t NV_BG     = t.color565(2, 16, 6);
    const uint16_t NV_MID    = t.color565(16, 55, 20);
    const uint16_t NV_BODY   = t.color565(55, 140, 55);
    const uint16_t NV_BRIGHT = t.color565(120, 235, 110);
    const uint16_t NV_GLOW   = t.color565(235, 255, 215);
    // Reflective eyeshine reads distinctly red on real trail-cam
    // footage for some species (per the actual lore) even under green
    // night vision — worth breaking the monochrome palette for.
    const uint16_t NV_RED    = t.color565(255, 40, 40);

    if (!inited) {
        for (uint8_t i = 0; i < N; i++) {
            walkers[i].x       = (float)random(0, w);
            walkers[i].speed   = 0.35f + (float)random(0, 100) / 100.0f * 0.5f;
            walkers[i].dir     = random(0, 2) ? 1 : -1;
            walkers[i].phase   = (float)random(0, 6283) / 1000.0f;
            walkers[i].h       = (int)(bandH * (0.30f + (float)random(0, 100) / 100.0f * 0.20f));
            walkers[i].species = (uint8_t)random(0, 7);
            walkers[i].nextJump = 0;
            walkers[i].glitchUntil = 0;
        }
        inited = true;
    }

    // Base wash + scanlines + static grain — full-band repaint every
    // frame, same as the other background styles, so nothing (walker
    // silhouettes included) ever leaves a stale pixel behind.
    t.fillRect(0, yStart, w, bandH, NV_BG);
    for (int y = yStart; y < yEnd; y += 3) t.drawFastHLine(0, y, w, NV_MID);
    for (uint8_t i = 0; i < 40; i++) {
        int gx = random(0, w);
        int gy = yStart + random(0, bandH);
        t.drawPixel(gx, gy, (random(0, 3) == 0) ? NV_BRIGHT : NV_MID);
    }

    int groundY = yEnd - 4;
    t.drawFastHLine(0, groundY, w, NV_MID);

    for (uint8_t i = 0; i < N; i++) {
        Walker& wk = walkers[i];
        wk.x += wk.dir * wk.speed;
        int pad = wk.h;
        // Re-roll which cryptid this slot is next time it wraps around,
        // so the cast keeps changing rather than looping the same one.
        if (wk.dir > 0 && wk.x > w + pad) { wk.x = (float)-pad; wk.species = (uint8_t)random(0, 7); wk.nextJump = 0; }
        if (wk.dir < 0 && wk.x < -pad)    { wk.x = (float)(w + pad); wk.species = (uint8_t)random(0, 7); wk.nextJump = 0; }
        ...
        switch (wk.species) {
            case 1: {
                // Mothman: no distinct head (lore-accurate — eyes set
                // into the chest/shoulders), broad flapping wings,
                // glowing red eyes, hovering rather than walking.
                int cy2 = groundY - hgt / 2 + (int)(sinf((float)now / 500.0f + wk.phase) * hgt * 0.08f);
                float flap = sinf((float)now / 220.0f + wk.phase);
                int wingSpan = (int)(hgt * 0.9f);
                int wingDip  = (int)(flap * hgt * 0.15f);
                t.fillTriangle(cx, cy2 - hgt / 6, cx - wingSpan, cy2 - hgt / 8 - wingDip, cx - hgt / 8, cy2 + hgt / 6, NV_BODY);
                t.fillTriangle(cx, cy2 - hgt / 6, cx + wingSpan, cy2 - hgt / 8 - wingDip, cx + hgt / 8, cy2 + hgt / 6, NV_BODY);
                t.fillTriangle(cx, cy2 - hgt / 3, cx - hgt / 10, cy2 + hgt / 3, cx + hgt / 10, cy2 + hgt / 3, NV_BODY);
                t.fillCircle(cx - 2, cy2 - hgt / 4, 2, NV_RED);
                t.fillCircle(cx + 2, cy2 - hgt / 4, 2, NV_RED);
                break;
            }
            case 2: {
                // Dogman: Bigfoot's biped walk cycle, but with a
                // pointed snout and upright ears instead of a round
                // head — the Michigan Dogman's defining silhouette.
                int headY = groundY - hgt, shoulderY = groundY - (hgt * 3) / 4, hipY = groundY - hgt / 5;
                float legPhase = (float)now / 140.0f * wk.speed + wk.phase;
                int legSwing = (int)(sinf(legPhase) * hgt * 0.18f);
                int armSwing = -legSwing;
                t.drawLine(cx, hipY, cx - legSwing, groundY, NV_BODY);
                t.drawLine(cx, hipY, cx + legSwing, groundY, NV_BODY);
                t.fillTriangle(cx, headY + hgt / 6, cx - hgt / 5, hipY, cx + hgt / 5, hipY, NV_BODY);
                t.drawLine(cx, shoulderY, cx + armSwing, shoulderY + hgt / 3, NV_BODY);
                t.drawLine(cx, shoulderY, cx - armSwing, shoulderY + hgt / 3, NV_BODY);
                int headR = hgt / 7; if (headR < 2) headR = 2;
                t.fillCircle(cx, headY + headR, headR, NV_BODY);
                t.fillTriangle(cx + wk.dir * headR, headY + headR, cx + wk.dir * headR * 2, headY + headR - 1, cx + wk.dir * headR * 2, headY + headR + 2, NV_BODY);
                t.fillTriangle(cx - headR / 2, headY, cx - headR, headY - headR / 2, cx - headR / 4, headY - headR / 3, NV_BODY);
                t.fillTriangle(cx + headR / 2, headY, cx + headR, headY - headR / 2, cx + headR / 4, headY - headR / 3, NV_BODY);
                t.drawPixel(cx - 1, headY + headR, NV_GLOW);
                t.drawPixel(cx + 1, headY + headR, NV_GLOW);
                break;
            }
            case 3: {
                // Chupacabra: low, hunched, hopping gait, spikes down
                // the spine, red eyes close to the ground.
                int bodyLen = hgt, bh = hgt / 2;
                float hop = fabsf(sinf((float)now / 220.0f * wk.speed + wk.phase));
                int by = groundY - bh / 2 - (int)(hop * bh * 0.35f);
                t.fillEllipse(cx, by, bodyLen / 3, bh / 3, NV_BODY);
                for (uint8_t k = 0; k < 4; k++) {
                    int sxk = cx - bodyLen / 4 + k * (bodyLen / 6);
                    t.fillTriangle(sxk, by - bh / 3, sxk - 2, by - bh / 3 - 4, sxk + 2, by - bh / 3 - 4, NV_BODY);
                }
                int legY = groundY - (int)(hop * 4);
                t.drawLine(cx - bodyLen / 6, by + bh / 4, cx - bodyLen / 6, legY, NV_BODY);
                t.drawLine(cx + bodyLen / 6, by + bh / 4, cx + bodyLen / 6, legY, NV_BODY);
                int headR = bh / 3; if (headR < 2) headR = 2;
                int headX = cx + wk.dir * (bodyLen / 4);
                t.fillCircle(headX, by - bh / 6, headR, NV_BODY);
                t.fillCircle(headX + wk.dir, by - bh / 6, 1, NV_RED);
                break;
            }
            case 4: {
                // "Long Horse"-style long-legs cryptid: barely any
                // body — it sits up near the top, and the "legs" (six
                // long, splayed, multi-jointed limbs) make up almost
                // the whole silhouette, rippling in an insectoid gait.
                int bodyY = groundY - hgt;
                int bodyR = hgt / 10; if (bodyR < 2) bodyR = 2;
                t.fillCircle(cx, bodyY, bodyR, NV_BODY);
                static const uint8_t NL = 6;
                for (uint8_t k = 0; k < NL; k++) {
                    float legPhase = (float)now / 180.0f + k * 1.05f + wk.phase;
                    float spread = (k - NL / 2.0f) * 0.35f;
                    int kneeX = cx + (int)(sinf(spread) * hgt * 0.5f);
                    int kneeY = bodyY + (int)(hgt * 0.4f + sinf(legPhase) * hgt * 0.06f);
                    int footX = cx + (int)(sinf(spread) * hgt * 0.85f);
                    int footY = groundY - (int)(fabsf(sinf(legPhase)) * hgt * 0.05f);
                    t.drawLine(cx, bodyY, kneeX, kneeY, NV_BODY);
                    t.drawLine(kneeX, kneeY, footX, footY, NV_BODY);
                }
                t.drawPixel(cx - 1, bodyY, NV_GLOW);
                t.drawPixel(cx + 1, bodyY, NV_GLOW);
                break;
            }
            case 5: {
                // Slenderman: faceless (deliberately no eye pixels —
                // every other species has glowing eyes, this one
                // pointedly doesn't), unnaturally thin, barely moves
                // beyond a slow sway, in a shadowy silhouette a shade
                // darker than the usual NV green rather than the
                // brighter walker body tone.
                uint16_t shadowCol = blend(NV_BG, NV_BODY, 90);
                float sway = sinf((float)now / 1600.0f + wk.phase) * 1.5f;
                int bodyX  = cx + (int)sway;
                int headY  = groundY - hgt, hipY = groundY - hgt / 6;
                t.fillRect(bodyX - hgt / 16, headY + hgt / 6, hgt / 8, hgt - hgt / 6, shadowCol);
                int headR = hgt / 9; if (headR < 2) headR = 2;
                t.fillEllipse(bodyX, headY + headR, headR, (int)(headR * 1.3f), shadowCol);
                // One arm slowly, unnaturally over-extends and retracts.
                float stretch = 0.5f + 0.5f * sinf((float)now / 2600.0f + wk.phase * 2.0f);
                int armLen = (int)(hgt * (0.5f + stretch * 0.5f));
                t.drawLine(bodyX, headY + hgt / 3, bodyX - armLen / 2, hipY, shadowCol);
                t.drawLine(bodyX, headY + hgt / 3, bodyX + armLen / 2, hipY, shadowCol);
                break;
            }
            case 6: {
                // Anomalous statue entity: rigid, jagged, motionless
                // pose — it doesn't walk. Instead it sits still, then
                // every few seconds snaps to a new nearby spot in a
                // single instantaneous jump with a burst of static, the
                // "you looked away and it moved" trope.
                if (now >= wk.nextJump) {
                    wk.x += wk.dir * (float)random(20, 50);
                    wk.nextJump    = now + (uint32_t)random(2500, 6000);
                    wk.glitchUntil = now + 200;
                }
                int headY = groundY - hgt, hipY = groundY - hgt / 5;
                t.fillTriangle(cx, headY + hgt / 6, cx - hgt / 6, hipY, cx + hgt / 6, hipY, NV_BODY);
                t.drawLine(cx - hgt / 8, headY + hgt / 3, cx + hgt / 10, headY + hgt / 2, NV_MID);
                t.drawLine(cx + hgt / 8, hipY - hgt / 6, cx - hgt / 6, hipY, NV_MID);
                t.drawLine(cx - hgt / 6, headY + hgt / 4, cx - hgt / 5, hipY, NV_BODY);
                t.drawLine(cx + hgt / 6, headY + hgt / 4, cx + hgt / 5, hipY, NV_BODY);
                int headR = hgt / 7; if (headR < 2) headR = 2;
                t.fillCircle(cx, headY + headR, headR, NV_BODY);
                t.drawPixel(cx - 1, headY + headR, NV_GLOW);
                t.drawPixel(cx + 1, headY + headR, NV_GLOW);
                if (now < wk.glitchUntil) {
                    for (uint8_t g = 0; g < 10; g++) {
                        int gx = cx + random(-hgt / 3, hgt / 3);
                        int gy = groundY - random(0, hgt);
                        t.drawPixel(gx, gy, random(0, 2) ? NV_BRIGHT : NV_GLOW);
                    }
                }
                break;
            }
    ...
    // Corner-cut vignette — a cheap stand-in for the circular NV-goggle
    // tunnel (a true circular mask isn't worth the per-pixel cost here).
    int cut = bandH / 4;
    t.fillTriangle(0, yStart, cut, yStart, 0, yStart + cut, BLACK);
    t.fillTriangle(w - 1, yStart, w - 1 - cut, yStart, w - 1, yStart + cut, BLACK);
    t.fillTriangle(0, yEnd - 1, cut, yEnd - 1, 0, yEnd - 1 - cut, BLACK);
    t.fillTriangle(w - 1, yEnd - 1, w - 1 - cut, yEnd - 1, w - 1, yEnd - 1 - cut, BLACK);

    // HUD readout, blinking REC dot.
    t.setTextSize(1);
